      ...
      <div className="max-w-2xl mx-auto">
        ...
        {showScanner && !loading && (
          <div className="card mb-6">
            <h2 className="text-xl font-semibold mb-4">Scan Gatepass QR Code</h2>
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p>{formatDateTime(gatepassInfo.leave_time)}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600">Expected Return</label>
                  <p>{formatDateTime(gatepassInfo.expected_return_time)}</p>
                </div>
              </div>

              {gatepassInfo.actual_exit_time && (
                <div>
                  <label className="block text-sm font-medium text-gray-600">Exit Time</label>
                  <p>{formatDateTime(gatepassInfo.actual_exit_time)}</p>
                </div>
              )}

              {gatepassInfo.actual_return_time && (
                <div>
                  <label className="block text-sm font-medium text-gray-600">Return Time</label>
                  <p>{formatDateTime(gatepassInfo.actual_return_time)}</p>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3 mt-6">
              <div className="flex gap-4">
                ...
                {gatepassInfo.actual_return_time && (
                  <div className="bg-blue-100 border border-blue-400 text-blue-700 px-4 py-3 rounded w-full text-center">
